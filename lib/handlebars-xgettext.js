...
function parse(options, callback) {
  ...
  readdirp({root: dir}, function(err, res) {
    ...
    res.files.forEach(function (file) {
      cleanPath = file.path.split(path.sep).join('/');
      strings[cleanPath] = parser.parse(fs.readFileSync(file.fullPath, options['from-code']));
    });
    ...
    for (file in strings) {
      for (str in strings[file]) {
        gettext.setTranslation(null, null, str);

        comment = _.extend({code: ''}, gettext.getComment(null, null, str));
        if (comment.code) {
          comment.code += '\n';
        }
        comment.code += file + ':' + strings[file][str];

        gettext.setComment(null, null, str, comment);
      }
    }
