var fs = require('fs'),
  ...
  Parser = require('gettext-handlebars'),
 ...
 */
function parse(input, options, cb) {
  options = options || {};
  ...
  options.keyword = options.keyword || ['_', 'gettext', 'ngettext:1,2'];
  ...
  var parser = new Parser(options.keyword.reduce(addToSpec, spec)),
    context = {};

  var parseTemplate = function (template, linePrefixer) {
      ...
      async.parallel(files.map(function (file) {
          return function (cb) {
              fs.readFile(path.resolve(file), options['from-code'], function (err, res) {
                  ...
                  parseTemplate(res, addPath(file.replace(/\\/, '/')));
  ...
  if (options.directory) {
    ...
    if (typeof input === 'string') {
      parseTemplate(input, function (line) {
